...
def build_optimizer(model, optim_cfg):
    if optim_cfg.OPTIMIZER == 'adam':
        ...
        optimizer = OptimWrapper.create(
            optimizer_func, 3e-3, get_layer_groups(model), wd=optim_cfg.WEIGHT_DECAY, trud_wd = True, bn_wd = True
        )
    else:
...
def build_scheduler(optimizer, total_iters_each_epoch, total_epochs, last_epoch, optim_cfg):
    decay_step = [x * total_iters_each_epoch for x in optim_cfg.DECAY_STEP_LIST]
    ...
    if optim_cfg.OPTIMIZER == 'adam_onecycle':
        lr_shceduler = OneCycle(
            optimizer, total_steps, optim_cfg.LR, list(optim_cfg.MOMS), optim_cfg.DIV_FACTOR, optim_cfg.PCT_START
        )
    else:
        lr_shceduler = lr_sched.LambdaLR(optimizer, lr_lbmd, last_epoch=last_epoch)
