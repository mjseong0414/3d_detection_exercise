...
class PointFeatureEncoder(object):
    ...
    def forward(self, data_dict):
        data_dict['points'], used_lead_xyz = getattr(self, self.point_encoding_config.encoding_type)(
            data_dict['points']
        )
        data_dict['use_lead_xyz'] = used_lead_xyz
        return data_dict
    
    def absolute_coordinated_encoding(self, points = None):
        if points is None:
        ...
        for x in self.used_feature_list:
            if x in ['x', 'y', 'z']:
                continue
            idx = self.src_feature_list.index(x) # idx에 현재 x의 index 저장
